...
logging.basicConfig(
    filename='predict.log',  # Tên file ghi kết quả
    filemode='w',  # 'w' để ghi đè file mỗi lần chạy, 'a' để ghi thêm vào cuối file
    format='%(asctime)s - %(levelname)s - %(message)s',  # Định dạng thông tin ghi
...
class ToTensor:
    ...
    def __call__(self, im):
        ...
        logger.info(f'Image transformed to tensor with shape: {im.shape}, dtype: {im.dtype}, max value: {im.max()}')
        return im
